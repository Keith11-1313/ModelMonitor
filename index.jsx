...
const RELEASE_TYPES = new Set(["model_release", "release", "preview_release", "open_source_release", "open_weight_release", "open_weights_release"]);
...
function liveSnapshot(models, availability, metadata, events, modelMap, now) {
  const identities = models.filter(canonicalRecord);
  const releases = events.filter(event => notableRelease(event, modelMap, now));
...
function Overview({ models, events, availability, metadata, now, state, modelMap, onSelect, report: loadedReport, onMarkSeen, cutoff }) {
  const report = loadedReport?.schemaVersion === 2 ? loadedReport : null;
  const snapshot = liveSnapshot(models, availability, metadata, events, modelMap, now);
  const releases = events.filter(event => notableRelease(event, modelMap, now)).sort((a, b) => b.date.localeCompare(a.date));
  const ready = key => (key.includes("Endpoints") ? ["availability", "metadata"] : key.startsWith("notableReleases") ? ["timeline", "models"] : ["models"]).every(dataset => state[dataset].status === "ready");
  ...
  return <><header className="overview-heading"><div><h1>ModelMonitor Intelligence Brief</h1><p>Dataset build: {date(metadata.generatedAt)}. Report generated: {date(report?.generatedAt)}.</p><p>A quick read of what changed, what is available now, and what still needs verification.</p></div></header>
    ...
    <section className="section"><h2>Current ecosystem snapshot</h2><p className="muted"><Tag>Derived</Tag> Recalculated from the records loaded in this browser. An endpoint counts as current only when its source evidence is healthy and recent.</p><div className="stats">{Object.entries(SNAPSHOT_LABELS).map(([key, label]) => <a href={key.includes("Endpoints") ? "#free" : /open|Source|Weights/i.test(key) ? "#open" : "#releases"} key={key}><span className="stat-value">{ready(key) ? number(snapshot[key]) : "Unknown"}</span><strong>{label}</strong><small>{ready(key) ? "Loaded evidence only" : "Required dataset not loaded"}</small></a>)}</div>{report && <details><summary>Published report snapshot (historical, not live)</summary><p>As of {report.periodEnd || report.generatedAt}. These saved counts may no longer satisfy freshness checks.</p><RecordFields record={report.snapshot} /><RecordFields record={report.health} /></details>}<p className="footnote"><a href="#analysis">Inspect source health and methodology</a>. Unknown pricing is neither free nor paid.</p></section>
    <section className="section"><div className="section-toolbar"><h2>Important releases</h2><a href="#releases">See release catalog</a></div><p className="muted">Recent notable releases that have a dated source. The full historical story lives in LLM History.</p>{releases.length ? <EventList events={releases.slice(0, 8)} {...common} /> : <Empty>No sourced notable releases in the loaded timeline. This is a coverage gap, not evidence that no releases occurred.</Empty>}</section>
